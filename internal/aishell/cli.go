...
Image builds use a configurable base image (Dockerfile FROM). Configure defaults and aliases:
  ai-shell config set-default-base-image <alias>
  ai-shell config alias set <alias> <image> <family>
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !noBuild {
				...
				base = chosenBase
				// family is not needed here — the Dockerfile was already generated by init
			}
...
`, out))
	...
	cmd.Flags().StringVar(&baseImage, "base-image", "", "Base image alias (defined via 'ai-shell config alias set'). Can also be provided as an optional positional argument.")
...
func newConfigCmd() *cobra.Command {
	...
	aliasSetCmd := &cobra.Command{
		Use:   "set <alias> <image> <family>",
		Short: "Set an alias for a base image (family: apt, dnf, zypper, apk)",
		Args:  cobra.ExactArgs(3),
		RunE: func(cmd *cobra.Command, args []string) error {
			key := strings.TrimSpace(args[0])
			img := strings.TrimSpace(args[1])
			family := strings.TrimSpace(args[2])
			...
			cfg.BaseImageAliases[key] = AliasEntry{Image: img, Family: family}
			if err := writeConfig(cfg); err != nil {
				return err
			}
			fmt.Printf("OK: alias %s=%s (family=%s)\n", key, img, family)
			return nil
		},
	}
	...
	aliasLsCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, k := range keys {
				kW = max(kW, len(k))
				iW = max(iW, len(cfg.BaseImageAliases[k].Image))
			}
			fmt.Printf("%-*s  %-*s  FAMILY\n", kW, "ALIAS", iW, "IMAGE")
			for _, k := range keys {
				e := cfg.BaseImageAliases[k]
				fmt.Printf("%-*s  %-*s  %s\n", kW, k, iW, e.Image, e.Family)
			}
			return nil
		},
	}
...
- Dockerfile (auto-generated section with markers + customization section)
- docker-compose.yml
- bootstrap-tools.sh (runtime customization hook)
- setup-git-ssh.sh
- README.md

The base image must be specified as an alias (defined via 'ai-shell config alias set').
Each alias carries a package-manager family (apt, dnf, zypper, apk) that determines
which Dockerfile template is used.
...
`),
	...
	cmd.Flags().StringVar(&baseImage, "base-image", "", "Base image alias (defined via 'ai-shell config alias set'; default: from config)")
...
`),
	...
	cmd.Flags().StringVar(&workdir, "workdir", "", "Target workdir (default: current directory)")
	cmd.Flags().StringVar(&baseImage, "base-image", "", "Base image alias to embed as the default build arg in docker-compose.yml (required)")
